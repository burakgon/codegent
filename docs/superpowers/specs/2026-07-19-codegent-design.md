...
| running → +input flag | CC Notification/PermissionRequest hooks / Codex hooks / idle heuristic (silent, fallback-only) | renders in Waiting column; push (question/permission only); timer |
...
**Completion truth table (v2 — verified against real hook semantics):** Claude Code fires `Stop` at the end of **every turn** (including question-only turns), so Stop alone never signals completion. Review is gated **only** by `task_complete`: `task_complete` (+ Stop) → review. Stop without `task_complete` → input-needed(question) — the ordinary mid-conversation state. `StopFailure` (API-error turns) → error. Permission-request notifications → input-needed(permission). User Esc emits no hook — covered by PTY-write tracking (the daemon knows it sent the interrupt). Codex maps through its official hooks; the silence heuristic is fallback-only. **Contract-churn guard:** version probe at adapter start + nightly contract tests against real CLIs; unknown agent versions run in a flagged degraded mode.
...
**Input sub-kinds (adopted):** `question` and `permission` (distinct icon + push copy), `silent` (idle>T heuristic; strip-only, never pushes).
...
**Claude Code (primary):** orchestrator spawns interactive `claude` in the worktree PTY with `--settings` + `--mcp-config` supplied externally (repo never dirtied). Hooks: `SessionStart` (context injection), `Notification` (permission/question → input flag), `Stop` (completion evaluation). Prompt template = card title + description + instruction to call `task_complete` when done. **Sandboxed by default:** "automatic" mode runs agents inside their **native sandboxes** (Claude Code sandbox mode; Codex `workspace-write`) — autonomous and contained; the README line is "autonomous by default, sandboxed by default". **Host/YOLO mode** (dangerously-skip-permissions on the real machine) is a first-class, easily reachable switch — one click in the add-project sheet and per-card via ⋯, single confirm, clearly labeled, no digging. "ask" mode remains for the cautious; its permission events classify as `permission` input.

**Codex:** MCP via `config.toml`; lifecycle via Codex's **official hooks** (SessionStart / permission-request / Stop-class events, session id on stdin). The legacy `notify` interface is not used — it only reports turn completion and never surfaces approvals. Silence heuristic fallback-only.

**Universal terminal-state tier (third adapter class — replaces ACP; decision 2026-07-19):** every recognizable agent runs in its own TUI and is classified from outside by a layered, content-free stack **adopted from proven systems, never invented here** (herdr `b580103`, Orca `80e6322` — mechanisms, thresholds, and polarity per `docs/research/herdr-agent-state.md` and `docs/research/orca-agent-state.md`):

1. **Process-tree identity** — probe the pane shell's foreground process group, unwrap runtime wrappers (node/bun/nix/shell), recognize the agent binary (herdr: 300ms–5s cadence, 6 consecutive misses before "gone"; Orca: shared 500ms-TTL `ps` snapshot, 750/2000/3000/15000ms tiers).
2. **OSC title/progress classification** — braille-spinner range U+2800–U+28FF ⇒ working; per-agent idle glyphs (e.g. Claude `✳`); OSC `9;4` progress states — protocol codes and glyph ranges, not prose.
3. **Manifest-scoped blocked patterns** — narrow, priority-ordered `contains`/`regex` rules over the bottom screenful per agent manifest; **unknown screens classify as idle, never blocked** (strict-positive polarity).
4. **Output-quiescence as a readiness gate only, never a primary signal** — herdr shipped byte-quiescence-first and reverted it in two days (spinners are never byte-quiet; typing/resize are indistinguishable from work). Completion is cross-confirmed (debounce + consecutive idle process samples + no-child-processes veto + source dedup à la Orca).

Classification runs entirely daemon-side and emits **state enums only** — captured text never reaches the UI (principle 1). MCP-capable agents additionally get the three task tools; `task_complete` gates review exactly as in the premium tier. Agent manifests are plugin kind (a) (§12): community-maintained, remote-updatable, locally overridable — core does not own a per-agent pattern treadmill. Adopted semantics: `done = idle ∧ not-yet-viewed` (herdr) feeds unread state; on cold restart, an agent pane with a captured native session id relaunches via native resume (e.g. `claude --resume`) instead of ring replay.

**Premium-tier hardening (same research):** hook silence ≠ idle; a screen-detected blocker overrides hook state (herdr demoted CC hooks to session-id-only precisely because hooks miss permission outcomes and Esc interrupts); keystroke inference (Esc/Ctrl+C with a ~500ms settle, Orca) synthesizes the interrupt/answered transitions agents never emit hooks for.

**ACP status:** demoted out of core — ACP mode replaces the agent's own TUI with a protocol-rendered session, the opposite of principle 2, and both reference systems solved terminal-resident orchestration without it. Revisit only if agents adopt ACP natively with TUI-preserving semantics; a future ACP client would arrive as a plugin, not core.
...
- **PR tracking:** after Open PR the card carries a PR badge + link. Branch-ancestry auto-detection is unreliable under squash/rebase merges, so the primary control is an explicit **"mark merged"** action on the card; best-effort assist via `git cherry`/patch-id when it works. A read-only **CI checks chip** (via `gh` when available) shows check status in the review header — merging blind to CI is the alternative. Full checks panel stays on the roadmap.
...
2. **Hook reliability:** verify CC `Stop`/`Notification` firing patterns and Codex `notify` coverage against the truth table; tune the silent-idle heuristic threshold.
