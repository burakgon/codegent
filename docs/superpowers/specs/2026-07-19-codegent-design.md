...
- **License:** AGPL-3.0. Free for everyone to use (including at work); anyone distributing modified versions or offering it as a service must publish their source — blocks closed-source forks and competing SaaS. Copyright stays with the owner; contributions accepted under a CLA, keeping a future enterprise/commercial licensing option open (no pricing model now). **Platforms:** macOS + Linux native; Windows via WSL (documented). **Agents in v1:** Claude Code (deepest, hooks) + Codex (official hooks) + the **ACP adapter class** — any Agent Client Protocol agent (Gemini CLI, Copilot CLI, Goose, OpenCode, 30+) becomes orchestratable through one integration. Non-ACP CLIs still run in plain terminal sessions without orchestration guarantees.

## 2. Product principles (locked)

1. **Terminal content never leaves the terminal.** The daemon classifies state from deterministic signals (hooks, notify, exit codes) but the UI never displays scraped/derived content — no question previews, no error excerpts, no "exit 1". Surfaces show *state + elapsed time* only; clicking always routes to the terminal where the content lives.
...
**ACP (third adapter class):** an Agent Client Protocol client makes every ACP-speaking agent an orchestratable citizen — lifecycle, permission requests, and session management arrive over the standard protocol instead of bespoke integration. Study-first references: Zed's ACP client (origin implementation) and other production ACP clients (JetBrains); codegent gets listed in the ACP agent-registry ecosystem (distribution). CC/Codex hook adapters remain the premium tier (deepest truth-table fidelity); ACP agents get the standard tier.
...
- **v0.3:** relay + pairing (device confirm/revoke, signed UI manifest) + Web Push; review queue incl. stale cascade; **ACP adapter**; installer + `npx codegent`; Settings + first-run.
