...
**Codex:** MCP via `config.toml`; lifecycle via Codex's **official hooks** (SessionStart / permission-request / Stop-class events, session id on stdin). The legacy `notify` interface is not used — it only reports turn completion and never surfaces approvals. State fallback = the §6 universal stack (Codex's OSC behavior is well-charted: "Action Required" title ⇒ blocked, braille spinner ⇒ working — herdr's top-priority codex rules), never byte-quiescence.

### 6.1 State detection & completion doctrine (the system's main line — decision 2026-07-19, stable)

Every mechanism below is **adopted from proven production systems, never invented here** — herdr `b580103` and Orca `80e6322`; mechanisms, thresholds, and polarity are recorded with file:line evidence in `docs/research/herdr-agent-state.md` and `docs/research/orca-agent-state.md`. Those two docs are the implementation reference; this section is the contract. When implementation and this section disagree, this section governs; when this section is silent, the research docs' recorded choice is the default.
...
2. **OSC title/progress classification** — braille-spinner range U+2800–U+28FF ⇒ working; per-agent glyphs (Claude `✳` idle; Gemini `✋` permission / `✦⏲` working / `◇` idle); OSC `9;4` progress (`4;0` = done-idle); Codex literal "Action Required" title ⇒ blocked (top priority). Titles are volunteered metadata: retained ≤256 chars, sanitized, cleared on foreground-process change, matched server-side, never surfaced.
...
**Dispatch envelope (Orca-proven, all transports):** every dispatch carries card + attempt + dispatch ids; completions are deduped against the *live* dispatch so a stale retry can never complete current work; the preamble instructs "report completion exactly once, even on failure"; `task_progress` doubles as heartbeat — progress silence >10 min shows a soft warning on the card (never auto-fail; the §5 30-min runaway guard stands); 3 consecutive failed attempts → error(circuit-broken) and R1 moves on.

**Input-flag lifecycle:** *set* by hook events (premium), manifest blocked-rules (universal), or the silent rule (I2). *Cleared* by arbitration, not hope: answered/interrupt keystroke inference (Esc/Ctrl+C or submit with ~500ms settle and baseline revalidation — real hook racing in wins; per-agent quirks like double-Esc live in manifests), screen re-classification when the dialog leaves the screen, or the next explicit report. Manual override (`mark state` on the card, §7) is the escape hatch; the §9.2 watchdog cross-checks overrides against detection and flags mismatches.
